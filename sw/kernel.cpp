    ...
    uint32_t* queries_size, uint32_t* results_size, uint32_t* num_queries)
{
    ...
    {
        file->seekg(0, std::ios::end);
        uint raw_size = ((uint)file->tellg()) - 3 * sizeof(*queries_size);
        ...
        printf("> Queries size: %u Mb\n", *queries_size);
        printf("> Results size: %u Mb\n", *results_size); fflush(stdout);
                ...
                uint32_t& nfadata_size, uint32_t& queries_size, uint32_t& results_size)
{
    ...
    xcl_set_kernel_arg(kernel, 2, 4, &results_cls);
    xcl_set_kernel_arg(kernel, 3, 4, &extraParam);
    xcl_set_kernel_arg(kernel, 4, 4, &extraParam);
    xcl_set_kernel_arg(kernel, 5, sizeof(cl_mem), &buffer_nfadata);
...
int main(int argc, char** argv)
{
    ...
    cl_kernel        kernel = xcl_get_kernel(program, "ederah_kernel");
    ...
    run_nfabre_kernel(queue, world, program, kernel, nfa_data->data(), queries->data(), results.data(), 
                      nfadata_size, queries_size, results_size);
