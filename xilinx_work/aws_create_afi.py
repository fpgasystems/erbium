...
string = os.environ["VITIS_DIR"]
for i in range(0, length):
    krnl_name = krnl_names[i].replace('.xclbin', '')
    os.system(string + "/tools/create_vitis_afi.sh -s3_bucket=" + s3_bucket + " -s3_dcp_key=" + s3_dcp_key + " -s3_logs_key=" + s3_logs_key + " -xclbin=" + krnl_name + ".xclbin -o=" + krnl_name)